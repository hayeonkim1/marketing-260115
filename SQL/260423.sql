...
#2. products : 상품 정보 -1 (brand_id, product_id), (상품에 대한 대락적 정보=상품명,상품상세 url)
#3. products_metrics : 상품 정보 -2 (prouduct_id)(판매가, 할인가, 할인률, 평점)
#4. products_reviews : 상품정보 -3 (product_id) (리뷰 텍스트)
...
CREATE TABLE product_metrics(
	metirc_id INT AUTO_INCREMENT PRIMARY KEY,
    product_id INT NOT NULL,
    crawl_at DATETIME NOT NULL,
    original_price INT DEFAULT 0,  # 혹시 값이 없을 경우 디폴트인 0 을 준다.
...
CREATE TABLE product_reviews(
    ...
    CONSTRAINT uq_product_review UNIQUE(product_id, review_seq, crawl_at)  # 동일 상품의 리뷰이며 ()안의 값들이 같으면 한번만 받는다. 
    );
