...
// Bold-range text fill (character styles Bold / Black / weight ≥ 600)
server.tool(
  "set_bold_text_fill_color",
  "Within a subtree, set text fill only on bold/heavy character ranges (font style contains Bold or Black, or variable weight ≥ 600). Does not change non-bold letters. Pass a page or frame nodeId.",
  {
    nodeId: z.string().describe("Root node (PAGE, FRAME, GROUP, etc.) whose descendant TEXT nodes are scanned"),
    r: z.number().min(0).max(1).describe("Red component (0-1)"),
    g: z.number().min(0).max(1).describe("Green component (0-1)"),
    b: z.number().min(0).max(1).describe("Blue component (0-1)"),
    a: z.number().min(0).max(1).optional().describe("Alpha component (0-1)"),
  ...
  async ({ nodeId, r, g, b, a }: any) => {
    try {
      const result = (await sendCommandToFigma("set_bold_text_fill_color", {
        nodeId,
        color: { r, g, b, a: a ?? 1 },
      })) as { totalBoldRanges?: number; results?: { rangesColored: number }[] };
      const n = result?.totalBoldRanges ?? 0;
      return {
        content: [
          {
            type: "text",
            text: `Applied bold-only fill RGBA(${r}, ${g}, ${b}, ${a ?? 1}) under "${nodeId}": ${n} bold span(s) updated.`,
          },
        ],
      };
    } catch (error) {
      return {
        content: [
          {
            type: "text",
            text: `Error setting bold text fill: ${error instanceof Error ? error.message : String(error)}`,
          },
        ],
      };
    }
