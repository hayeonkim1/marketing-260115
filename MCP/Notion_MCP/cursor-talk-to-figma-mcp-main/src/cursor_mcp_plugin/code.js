...
async function handleCommand(command, params) {
  switch (command) {
    ...
    case "set_bold_text_fill_color":
      return await setBoldTextFillColor(params);
...
function fontNameIsBold(fontName) {
  if (!fontName || fontName === figma.mixed) return false;
  const s = (fontName.style || "").toLowerCase();
  return s.includes("bold") || s.includes("black");
}

function indexUsesBoldWeight(node, idx) {
  const fontName = node.getRangeFontName(idx, idx + 1);
  if (fontName === figma.mixed) return false;
  if (fontNameIsBold(fontName)) return true;
  if ("getRangeFontWeight" in node && typeof node.getRangeFontWeight === "function") {
    try {
      const w = node.getRangeFontWeight(idx, idx + 1);
      if (typeof w === "number" && !Number.isNaN(w) && w >= 600) return true;
    } catch (e) {
      /* ignore */
    }
  }
...
async function loadFontsForTextRange(node, start, end) {
  const seen = new Set();
  for (let k = start; k < end; k++) {
    const f = node.getRangeFontName(k, k + 1);
...
async function setBoldTextFillColor(params) {
  const {
    nodeId,
    color: { r, g, b, a },
  } = params || {};
  ...
  async function processTextNode(node) {
    ...
    const detail = { id: node.id, name: node.name, rangesColored: 0 };
    if (len === 0) return detail;

    let i = 0;
    while (i < len) {
      if (!indexUsesBoldWeight(node, i)) {
        i++;
        continue;
      }
      let j = i + 1;
      while (j < len && indexUsesBoldWeight(node, j)) {
        j++;
      }
      try {
        await loadFontsForTextRange(node, i, j);
        node.setRangeFills(i, j, [paint]);
        detail.rangesColored++;
      } catch (e) {
        console.error("setBoldTextFillColor setRangeFills failed", node.id, i, j, e);
      }
      i = j;
    }
    return detail;
  ...
  async function walk(n, results) {
    if (n.type === "TEXT") {
      results.push(await processTextNode(n));
      return;
    }
    if ("children" in n) {
      for (const c of n.children) {
        await walk(c, results);
      }
    }
  }
  ...
  const totalBoldRanges = results.reduce((s, x) => s + x.rangesColored, 0);

  return {
    message: `Colored bold text spans in ${results.length} text layer(s).`,
    results,
    totalBoldRanges,
  };
}
