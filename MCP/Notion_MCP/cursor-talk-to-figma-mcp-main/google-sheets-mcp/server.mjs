import dotenv from "dotenv"; //.env 파일 읽어오게하는 모듈
import path from "path"; // 특정 파일 찾아오거나 불러올때 사용하는 모듈
import { fileURLToPath } from "url"; // ESM 모듈 전용 라이브러리 (특정 파일의 경로를 확인할때)
import { google } from "googleapis"; // 구글 스프레드시트 API 를 사용할 수 있도록 해주는 라이브러리
import { z } from "zod"; // z object definition -> 완성된 요소를 검증해주는 라이브러리
// #MCP서버로서 실행할 수 있도록 해주는 클래스 객체
import { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js";
// Cursor 라는 MCP서버와 GS가 양방향 통신을 할 수 있게 하는 라이브러리
...
// .env 파일안에 기입한 credentials파일을 읽어와서 구글 스프레드 시트에 가서
...
// mcp.json 파일안에 입력한 mcp 서버이름을 정상적으로 인지, 작동
const server = new McpServer({
  name: "google-sheets-mcp",
  version: "1.0.0",
});
//--> mcp 서버가 GS에 접속해서 데이터 읽어오게하기 위한 것
server.registerTool(
  "gsheets_read_range",
  {
...
//mcp 서버가 GS에 접속해서 행 추가 등의 데이터 추가 등 수행
server.registerTool(
  "gsheets_append_rows",
  {
...
// mcp 서버가 GS에 접속해서 기존 데이터를 덮어쓰게 하기 | 수정
server.registerTool(
...
// mcp 서버가 GS에 접속해서 특정 컬럼에 입력된 키워드들을 수집.분석해서 형태소 분소
server.registerTool(
...
// Cursor와 외부 AP가 정상적으로 양방향 통신을 하도록 하기 귀해 인스턴스 객체 생성
const transport = new StdioServerTransport();
// 위에서 만든 객체를 활용해서 실제 서버간 접속하게 함
